/* TP01P2Q11 - Ordenacao por Shellsort
 ...
 * */
   public void ordemShellsort () {
      ...
      /*
       * Metodo que efetua a insercao nos pseudos-arrays do Shellsort.
       * @param int cor cor do pseudo grupo
       * @param int h passo do shellsort
       * */
         public void insercaoPorCor (int cor, int h){
            for (int i = (h+cor); i < n; i = i+h){
               Instituicao tmp = array [i]; movimentacoes++;
               int j = i - h;
               boolean stop = false;
               while ((j >= 0) && (array[j].getUf().compareTo (tmp.getUf())) >= 0 && !stop) {
                  comparacoes++;
                  comparacoes++;
                  if (array[j].getUf().compareTo (tmp.getUf()) == 0){
                        comparacoes++;
       ...
       //main
        public static void main (String [] args)
        {
         ...
         try {
          Shellsort lista = new Shellsort (200);
         //abrir arquivo
          FileWriter arquivo = new FileWriter ("628510_shellsort.txt");
