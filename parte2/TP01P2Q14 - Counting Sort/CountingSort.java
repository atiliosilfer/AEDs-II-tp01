/* TP01P2Q13 - Ordenacao por CountingSort
 ...
 * */
  ...
  /*
   *  Funcao que retorna o maior elemento do array[]
   *  @param int maior elemento 
   * */
      public int getMaior (){
         movimentacoes++;
      ...
      /*
 * Algoritmo de ordenacao por irsercao
 * */
   public void ordemInsercao ()
   {
      for (int i = 1; i < n; i++)
      {
         ...
         boolean stop = false;
        comparacoes++;
        while ((j>= 0 && ((array[j]).getPeriodico() >= tmp.getPeriodico())) && !stop) {
         //testar se o cod mantedora sao iguais
          comparacoes = comparacoes+2;
          if (((array[j]).getPeriodico() ==
                      tmp.getPeriodico())) {
               comparacoes++;
             if ((tmp.getSigla()).compareTo (array[j].getSigla ()) < 0) {
               array [j+1] = array [j];
               movimentacoes++;
               j--;
             }//fim if
   ...
   * */ 
      public void ordemCounting () {
       //Array para contar o numero de ocorrencias de cada elemento
        int [] count = new int [getMaior()+1];
        Instituicao [] ordenado = new Instituicao [n];
    
       //Iniciar cada posicao do array de contagem
        for (int i = 0; i < count.length; count [i] = 0,i++);
       
       //Agora, o count [i] contem o numero de elementos iguais a i
        for (int i = 0; i < n; count[array[i].getPeriodico()]++, movimentacoes++, i++);
       
       //Agora, o count [i] comtem o numero de elementos menores ou iguais a i
        for (int i = 1; i < count.length; count [i] += count [i-1],movimentacoes++, i++);

       //Ordenado
        for (int i = n-1; i >= 0; ordenado[count[array[i].getPeriodico()]-1] = array[i],
             movimentacoes += 2,count[array[i].getPeriodico()]--,i--);

       //copiando para o array original
        for (int i = 0; i < n; array [i] = ordenado [i],movimentacoes++ , i++);
      }//fim ordemCounting
 ...
 //main
  public static void main (String [] args)
  {
   ...
   try {
    CountingSort lista = new CountingSort (200);
   //abrir arquivo
    FileWriter arquivo = new FileWriter ("628510_countingsort.txt");
   ...
   //ordenar lista
    lista.ordemCounting();
    //como a lista ja esta parcialmente ordenado, e Countingsort so pode ser usado para inteiros
    //foi usado o metodo de Insercao para desempatar os elementos com o mesmo Periodico a partir da sigla
    lista.ordemInsercao();
