/* TP01P2Q16 - Ordenacao por MergeSort
...
/*
 * Chamar metodo de ordenacao MergeSort recursivo
 * */
   public void ordemMerge () {
      int i = 0;
      Instituicao tmp [] = new Instituicao [n];
      ordemMerge (i, (n-1), tmp);
   }//fim ordemMerge
   
/*
 * Metodo para intercalar os arrays no MergeSort
 * @param int i - inicio do array
 * @param int meio - meio do array
 * @param j - fim do array
 * @param Instituicao [] tmp - array temporario que auxilia na ordenacao
 * */
   public void intercalar (int i, int meio, int j,Instituicao [] tmp) {
      ...
      for (int k = i; k <= j; k = k+1){
         ...
         }else if (tmp[itmp].getReceita() <= tmp [jtmp].getReceita()){
            comparacoes = comparacoes+2;
            comparacoes++;
            if (tmp[itmp].getReceita() == tmp [jtmp].getReceita()){
                comparacoes++;
               if (tmp[itmp].getSigla().compareTo(tmp [jtmp].getSigla()) < 0){
                  movimentacoes++;
                  array[k] = tmp [itmp++];
                  } else {array [k] = tmp [jtmp++]; movimentacoes++;}
            } else {array[k] = tmp [itmp++];movimentacoes++;}
         }else {
            comparacoes = comparacoes + 3;
            array [k] = tmp [jtmp++]; movimentacoes++;
         }//fim else
      }//fim for
   }//fim intercalar 
 ...
 * @param int i = posicao inicial do array
 * @param int j = ultima posicao do array
 * @param Instituicao tmp = array auxiliar
 * */
   public void ordemMerge (int i, int j, Instituicao [] tmp){
        if (i < j){
         int meio = (i+j)/2;
         ordemMerge (i, meio, tmp);
         ordemMerge (meio+1,j, tmp);
         intercalar (i,meio,j, tmp);
        }//fim if
   }//fim ordemMerge
 ...
 //main
  public static void main (String [] args)
  {
   ...
   try {
    MergeSort lista = new MergeSort (200);
   //abrir arquivo
    FileWriter arquivo = new FileWriter ("628510_mergesort.txt");
