/* TP01P2Q17 - Ordenacao por RadixSort
...
/*
 * Algoritmo para achar o maior valor de codigo
 * */
   public int getMax ()
   {
      int max = array[0].getCodigo();
      for (int i = 1; i < n; i++) {
         comparacoes++;
         if (array[i].getCodigo() > max){
            max = array[i].getCodigo();
            movimentacoes++;
         }//fim if
      }//fim for
      return max;
...
/*
 * Algoritmo para organizar o array de acordo com
 * num da posicao determinada pelo exp
 * @param exp - valor num no num
 * */
   public void ordenar (int exp){
      Instituicao output [] = new Instituicao [n];
      int i;
      int count [] = new int [10];

      for (i = 0; i < n; i++){
         count [(array[i].getCodigo()/exp)%10]++;
         movimentacoes++;
      }//fim for
      ...
      for (i = n-1; i >= 0; i--){
         output [count[(array[i].getCodigo()/exp)%10]-1] = array [i];
         count [(array[i].getCodigo()/exp)%10]--;
         movimentacoes++;
      }//fim for

      for (i = 0; i < n; i++){
         array [i] = output [i];movimentacoes++;
      }//fim for
   }//fim ordenar
 ...
 * */
   public void ordemRadix (){
   //achar o valor maior valor do codigo de instituicao
    int m = getMax ();
 ...
 //main
  public static void main (String [] args)
  {
   ...
   try {
    RadixSort lista = new RadixSort (200);
   //abrir arquivo
    FileWriter arquivo = new FileWriter ("628510_radixsort.txt");
