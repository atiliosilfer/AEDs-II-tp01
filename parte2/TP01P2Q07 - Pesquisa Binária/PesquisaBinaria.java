...
public class PesquisaBinaria
{
 ...
 /*
  * Funcao para indicar o numero de comparacoes durante toda pesquisa
  *
 ...
 * */
   public void ordemQuickSort (int esq, int dir)
   {
      ...
      while (i <=j)
       {
         while ((array[i].getSigla()).compareTo (pivo.getSigla()) < 0)
         {
            i++;
            comparacoes++;
         }
         while ((array[j].getSigla()).compareTo (pivo.getSigla()) > 0)
         {
            j--;
            comparacoes++;
         }
      ...
      if (esq < j)
         ordemQuickSort (esq,j);
      if (i < dir)
         ordemQuickSort (i, dir);
   }//fim ordemQuickSort
 ...
 * */
   public boolean pesquisarSigla (String sigla)
   {
    ...
    int dir = (getN()-1), esq = 0, meio;
     while (esq <= dir)
      {
         meio = (esq+dir)/2;
         if ((array[meio].getSigla()).compareTo (sigla) == 0)
            {
             resp = true;
             esq = getN ();
             comparacoes++;
            }
         else if ((array[meio].getSigla()).compareTo (sigla) < 0)
            {
             esq = meio + 1;
 ...
 //main
  public static void main (String [] args)
  {
   ...
   try {
    PesquisaBinaria lista = new PesquisaBinaria (200);
   //abrir arquivo
    FileWriter arquivo = new FileWriter ("628510_binaria.txt");
